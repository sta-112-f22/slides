...
2. Edit the code in the `table-one` chunk to examine a Table of your variables. Where the comment says `#add your variable of interest here` add your variable of interest.
...
6. Update the labels vector if you added any variables that were not in the original dataset
7. Run the `ggdag` chunk to create the causal diagram 
8. Run the `adjustment_set` chunk to see what variables you need to adjust for
9. Write out your final model based on this adjustment set
...
* Report a predicted change in retention based on a change in your variable for a school with Wake Forest's same characteristics along with the appropriate 95% Interval
...
4. Using the `wake_forest` dataset provided, predict how a specific change in this value would change the predicted first year retention rate. Include an appropriate 95% interval for this prediction
